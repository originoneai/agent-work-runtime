#!/usr/bin/env python3
"""Verify SEC-002 artifact conditions without counting pending secret safeguards."""
import argparse
...
EXPECTED = {"artifact_import_limit", "artifact_read_limit"}
...
def main():
    ...
    report = {"work_item": contract["work_item"], "contract_id": contract["contract_id"],
              ...
              "stage": "artifact_bounds", "stage_target": len(EXPECTED), "contract_target": len(ids),
              "stage_passed": False, "item_completed": False, "e4_completed": 0, "runs": []}
    commands = [
        (["cargo", "test", "-p", "awr-runtime", "--test", "security_artifact_bounds", "--locked", "--", "--nocapture"], 4 if os.name == "posix" else 3),
        (["cargo", "test", "-p", "awr-runtime", "--lib", "artifact::tests", "--locked", "--", "--nocapture"], 2 if os.name == "posix" else 1),
        (["cargo", "test", "-p", "awr-cli", "--test", "security_artifact_bounds", "--locked", "--", "--nocapture"], 2),
    ]
    observed = set()
    for index, (command, expected_count) in enumerate(commands, 1):
        ...
        count = sum(map(int, re.findall(r"test result: ok\. (\d+) passed; 0 failed; 0 ignored", output)))
