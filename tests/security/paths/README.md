...
符号链接替换检查目前使用 Unix 平台接口。其他平台缺失这些条件时，验证器会报告未覆盖，不把跳过计为通过；跨平台实际验证属于单独的平台门槛。各平台按实际运行结果判断覆盖情况。

Locator 解析与目录发现通过授权检查后，普通路径仍可能遭遇文件或祖先目录被替换。实现通过目录句柄固定路径，在逐层打开和目录枚举时禁止再次跟随符号链接。
