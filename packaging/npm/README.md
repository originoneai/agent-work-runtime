...
AWR 0.5.0 installs the native Rust `awr` CLI and `awr-mcp` server for stdio and shared Streamable HTTP.
...
```sh
npx --package=@originoneai/agent-work-runtime@0.5.0 awr --help
npx --package=@originoneai/agent-work-runtime@0.5.0 awr-mcp --project /absolute/project
```
...
The distribution targets macOS 15+ arm64 and Intel x64, Linux arm64 and x64 with glibc 2.39+ (Ubuntu 24.04
