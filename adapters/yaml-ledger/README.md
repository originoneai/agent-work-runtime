...
Git sources, scalar evidence, aliased targets, anchored/tagged target nodes, complex mapping keys and unsupported fields return `proposal_required`. Work status, owner, blocker, verification and evidence-list changes are reserved for domain actions and completion gates. Invalid values and no-op patches fail explicitly. Project runtime files under `.awr` cannot be mutation targets. Source and recovery snapshots are capped at 16 MiB.
