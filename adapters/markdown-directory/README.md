...
The adapter is read-only. The `index_directory` developer example can project files now. Reconciliation that marks removed source rows inactive is tracked by the incremental indexing item; the inventory API already reports those removals.
