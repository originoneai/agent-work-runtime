...
当前使用 `awr-core` 的秘密策略 3。组件合同 `tests/security/payloads/contract.json` 1.5.0 保持 32 个条件；AWR-TAKE-006 补充无凭据结构定义、Bearer 普通文字和伪装夹带检查。历史回执仍绑定各自当时的策略与合同。原始来源仍由项目维护者负责；AWR 不改写或删除包含敏感值的源文件。
