...
当前使用 `awr-core` 的秘密策略 2。组件合同 `tests/security/payloads/contract.json` 1.4.0 保持 32 个条件；QA-001 的集中回归补充 Basic 普通文字、短凭据和旧缓存的验证。SEC-002 原始回执仍绑定当时的策略 1 / 合同 1.3.0。原始来源仍由项目维护者负责；AWR 不改写或删除包含敏感值的源文件。
...
- FTS 策略为 4，首次读取时重建旧缓存，包括策略 3 下误删的普通 Basic 说明和漏检的短凭据。摘要检查完整字段后才取短文本；敏感摘要标为 `[redacted]`。身份、关联工作或来源引用含敏感值时，整条搜索文档不进入索引。查询参数也经过检查。
