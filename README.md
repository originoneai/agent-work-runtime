...
For an existing or empty project, `awr init` proposes a reviewable intake: existing sources remain authoritative, conventional Markdown ledgers can be indexed, and only missing goal/work sources are proposed under `.awr/intake/`. A primary YAML ledger can contain both goals and tasks; separate plans and rules are optional. Use `--goal "Project purpose" --accept` for a new project or `--write-draft /outside/project/intake.json` for review before acceptance. Generated intake work and draft goals do not imply business readiness.

Run `awr intake inspect --json` after organizing the sources. Init, status, ready and MCP status/ready return `organization`: explicit state, source/goal references, ordered actions with minimum fields, and a recheck command. An empty ledger, unreadable source, unfinished organization, blocked work and source-declared completion have different states. `organization.executable_work` adds goal/acceptance/action checks to dependency readiness. Only `--source-sha FULL_SHA` (or MCP `source_sha`) enables current completion-report verification; it never runs the recorded commands. See [organization and recheck](docs/TAKEOVER.md#organize-and-recheck).
